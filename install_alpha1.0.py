...
""" if os.getcwd() != installMainPath:   #如果工作目录不是安装程序本体所在目录
    os.chdir(installMainPath)            #就cd回本体所在目录
    print(f"工作目录已切换: {installMainPath}") """   #打包后运行时程序本体在临时目录, cd回去会炸(alpha1.1就是因为这个炸的)
...
def getConfig(name):
    ...
    if name not in os.listdir():   #如果工作目录下没有配置文件
        ...
        with open(file = name, mode = "r", encoding = "utf8") as fo:   #就开始读取
            ...
            """ if targetUsers == "":
                startPath = os.getenv("USERPROFILE") + "/AppData/Roaming/Microsoft/Windows/Start Menu/Programs/Startup/"
            else:
                startPath = "C:/Users/{}/AppData/Roaming/Microsoft/Windows/Start Menu/Programs/Startup/".format(targetUsers) """
